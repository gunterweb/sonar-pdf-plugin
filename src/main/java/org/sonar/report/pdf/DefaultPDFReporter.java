...
public class DefaultPDFReporter extends PDFReporter {
	...
	public DefaultPDFReporter(final Credentials credentials, final URL logo, final String projectKey,
	...
	protected void printPdfBody(final Document document) throws DocumentException, IOException, ReportException {
		...
		while (it.hasNext()) {
			...
			ChapterAutoNumber subprojectChapter = new ChapterAutoNumber(
					new Paragraph(getTextProperty("general.module") + ": " + subproject.getName(), Style.TITLE_FONT));
	...
	private void printDashboard(final Project project, final Section section) throws DocumentException {
	...
	private void printProjectInfo(final Project project, final Section section) throws DocumentException {
		List data = new List();
